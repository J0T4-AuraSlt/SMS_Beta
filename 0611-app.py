...
def send_email_notification(email, subject, message):
    with SMTP('mail.auraslt.com', 465) as smtp:
        ...
        smtp.sendmail('reset_pass@portal.sms-vivo.com', email, msg)
...
def manage_user():
    ...
    if action == 'enable':
        return handle_status_update(users, 1)
    elif action == 'disable':
        return handle_status_update(users, 0)
    elif action == 'reset_password':
        return handle_password_reset(users, new_password)
    else:
        return jsonify({"status": "error", "message": "Acción no válida"}), 400


def handle_status_update(users, new_status):
    conn = get_db_connection()
    cursor = conn.cursor()
...
def handle_password_reset(users, new_password):
    conn = get_db_connection()
    cursor = conn.cursor()
    for user_id in users:
